 ...
 *	1.7.20 - Optimized isST/isHE, fixed getChildName()
 */
...
def helperSmartAppsPage() {
	...
	LOG("The available Helper ${ST?'Smart':''}Apps are ${getHelperSmartApps()}", 5, null, "info")
 	...
 	dynamicPage(name: "helperSmartAppsPage", title: (HE?'<b>':'') + "Configured Helper ${ST?'Smart':''}Apps" + (HE?'</b>':''), install: true, uninstall: false, submitOnChange: true) { 
...
String getChildAppName(String childId) {
    ...
    children.each {
		//log.debug "label: ${it.label}, displayName: ${it.displayName}, id: ${it.id}, childId: ${childId}"
    	if (!childName && (it.id.toString() == childId)) {
        	childName = it.label.toString()
        }
    }
	//log.debug "childName: ${childName}"
	///***********************************
	/// Need to strip off the label (paused) if present!!!!
    return childName
}
